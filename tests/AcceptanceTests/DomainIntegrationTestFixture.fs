    ...
    open Raven.Abstractions.Data
        ...
        module DomainIntegrationTestFixture = 
            ...
            let ``Batch save and get`` () =    
                let bb = {Batch.BatchId=guidId; Batch.CreatedDate=DateTime.UtcNow; Batch.Id=Guid.NewGuid(); Batch.Name="surfReportBatch";Batch.Jobs=swJobs; Batch.Status=BatchStatus.Waiting;Batch.TriggerBuilder=TriggerBuilder.Create();}
                use session = Ioc.Resolve<IDocumentSession>()
                session.Store(bb)
                session.SaveChanges()
