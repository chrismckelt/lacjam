...
module DomainIntegrationTestFixture =
    ...
    [<Fact>]
    let ``Mongo is good ``() = _db |> should not' (Null)
    ...
    let ``Audit save and get``() =
        let aud =
            { Audit.Id = MongoDB.Bson.ObjectId.Empty;
              Audit.AuditType = "test";
              Audit.CreatedDate = DateTime.Now;
              Audit.Message = "test-message" }
