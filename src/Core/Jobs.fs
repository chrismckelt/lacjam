...
module Jobs =
    ...
    type JobMessage() =
        ...
        override this.ToString() =
            String.Format
                (" JobType: {0}   Id: {1}   BatchId: {2} CreatedId: {3} Payload: {4}  IsCompleted: {5}",
                 this.GetType().Name, this.Id.ToString(), this.BatchId.ToString(), this.CreatedDate.ToString(),
                 this.Payload, this.IsCompleted.ToString())
        interface IMessage
    ...
    type JobResult(jm : JobMessage, success : bool, result : string, ?resubmitTime : TimeSpan) =
        ...
        let getValue =
            if (rt.IsSome) then rt.Value
            else new TimeSpan()

        do
            if (success) then j.IsCompleted <- true

        member x.JobMessage
            with get () = j
            and set (value) = j <- value

        member x.JobResultId
            with get () = r
        ...
        member x.ResubmitTime
            with get () = (getValue)
            and set (v : TimeSpan) = rt <- (Some(v))

        override x.ToString() =
            String.Format
                ("Finished :: Job: {0} ResultId: {1} CreatedDate: {2} JobMessage: {3}  ResubmitTime: {4}",
                 x.JobMessage.GetType(), x.JobResultId, x.CreatedDate, x.JobMessage.ToString(), rt.Value.ToString())
        interface IMessage
    ...
    type AuditJob() =
        inherit JobMessage()
        member val Audit = None with get, set
        interface IMessage
    ...
    type SendEmailJob() =
        ...
        member val Email = { Email.To = "";
                             Email.From = "";
                             Email.Subject = "";
                             Email.Body = "" } with get, set
    ...
    type SendTweetJob() =
        inherit JobMessage()
        member val Settings = Settings.getTwitterSettings with get, set
...
type Batch =
    { mutable Id : Guid;
      ...
      Jobs : System.Collections.Generic.List<Jobs.JobMessage>;
      Status : BatchStatus;
      mutable TriggerName : string }

type IContainBatches =
    abstract Batches : Batch list with get

[<Serializable>]
type BatchSubmitterJob() =
    inherit Jobs.JobMessage()

    member val Batch = { Batch.BatchId = Guid.NewGuid();
                         Batch.CreatedDate = DateTime.Now;
                         Batch.Id = Guid.NewGuid();
                         Batch.Name = "SeedBatchJob";
                         Batch.Jobs = new System.Collections.Generic.List<Jobs.JobMessage>();
                         Batch.Status = BatchStatus.Waiting;
                         Batch.TriggerName = BatchSchedule.Daily.ToString() } with get, set
...
module JobHandlers =
    ...
    type JobResultHandler(log : Lacjam.Core.Runtime.ILogWriter) =
        ...
        interface NServiceBus.IHandleMessages<Jobs.JobResult> with
            member x.Handle(jr) =
                try
                    if (jr.Success) then log.Write(LogMessage.Info("Success -> Job Result : " + jr.ToString()))
                    else log.Write(LogMessage.Info("Failed -> Job Result : " + jr.ToString()))
                with ex -> log.Error("Error -> Job Result : " + jr.ToString(), ex)
            ...
            member x.Handle(job) =
                ...
                | _ ->
                    ...
                    try
                        ...
                        bus.Reply(jr)
                    with ex ->
                        log.Write(Error("PageScraperJobHandler -- " + job.ToString(), ex, true))
